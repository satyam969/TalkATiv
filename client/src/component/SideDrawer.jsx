...
const {user,setSelectedChat,setChats,chats,notifications,setNotifications}=useChat();
...
setSearchResult(response.data.data);
  ...
  return (
    <>
      <Box
      className="flex justify-between items-center bg-white w-100 borderWidth-[5px]">
...
<div>
  <Menu>
<MenuButton p={1} >
<span className={`ml-4`}>{notifications?.length}</span>
<BellIcon fontSize="2xl" m={1}/>
</MenuButton >

{/* some functionality */}
{/* agr selected chat hai already toh uska notification htate jao */}
...
<MenuItem key={n._id} onClick={(e)=>{

setSelectedChat(n.chat)
setNotifications(notifications.filter((no)=>no!==n))

}}>
{n.chat.isGroupChat? `New Message In ${n.chat.chatName}`: `New Message From ${getSender(user,n.chat.users)}`}</MenuItem>
